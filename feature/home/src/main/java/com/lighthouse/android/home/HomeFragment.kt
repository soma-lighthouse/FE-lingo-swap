...
class HomeFragment @Inject constructor() :
    ...
    private lateinit var adapter: SimpleListAdapter<MatchProfileVO, UserInfoTileBinding>
    private var profileList = mutableListOf<MatchProfileVO>()
    ...
    private fun render(uiState: UiState) {
        when (uiState) {
            ...
            is UiState.Success<*> -> {
                ...
                if (uiState.data is List<*> && (uiState.data as List<*>).all { it is MatchProfileVO }) {
                    profileList.add(MatchProfileVO())
                    profileList.addAll(uiState.data as List<MatchProfileVO>)
                    adapter.submitList(profileList)
                }
    ...
    private fun initAdapter() {
        adapter = makeAdapter(requireContext(), {
            viewModel.startChatting(it)
        }) { userId, name, region ->
